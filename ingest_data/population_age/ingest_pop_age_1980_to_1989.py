...
# Filepaths to the fixed-width text files containing population by single year of age for the years 1980-1989.
# Sourced from "Quarterly Intercensal Estimates of Resident Population plus Armed Forces overseas" section of
...
def read_and_transform_data(*filePaths):
    """
    Read in the data from each text file, transform it to fit the target schema, and concatenate the results into one list.
    ...
    * list : list
    ...
    # Stores the cumulative data as it is read in from the different text files and transformed.
    list = []
...
def transform_data(file):
    """
    Transform the data in the file to fit the target schema, and return the data as a list.
    ...
    * list : list
    ...
    # Stores the cumulative data as each line of the file is read and transformed.
    list = []
    ...
    while True:
        ...
        if (month == 7 and age != 999):
            ...
            # If age is 100, then append the summed population to the main list and reset the populationAge85OrGreater tracker to zero.
            if age == 100:
                list.append([year, 85, populationAge85OrGreater])
                populationAge85OrGreater = 0
            # Otherwise, append this list (representing the population of a specific age group for a given year) to the main list.
            else:
...
def main():
    data = read_and_transform_data(textFile1980to1981, textFile1981to1982, textFile1982to1983, textFile1983to1984, textFile1984to1985, textFile1985to1986, textFile1986to1987, textFile1987to1988, textFile1988to1989, textFile1989to1990)
